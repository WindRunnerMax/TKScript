...
const keyboardHandler = (e: KeyboardEvent) => {
  if (e.ctrlKey && e.key.toLocaleUpperCase() === "C") {
...
export const Generic: WebSite = {
  ...
  start(type) {
    if (type === COPY_TYPE) {
      ...
      EventBus.on(EVENTS_TYPE.KEY_BOARD_CAPTURE, keyboardHandler);
  ...
  close(type) {
    if (type === COPY_TYPE) {
      ...
      EventBus.off(EVENTS_TYPE.KEY_BOARD_CAPTURE, keyboardHandler);
    } else if (type === CONTEXT_MENU_TYPE) {
